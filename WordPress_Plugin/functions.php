 ...
 * Description: Easily display temperatures, humidity, and pressure taken by your Raspberry Pi.
 * Version: 1.5
...
function visualization_line_chart_shortcode($atts, $content = null)
{
	...
	$display                = esc_sql($options[display]); //do we show only temp, only humidity, only pressure or all?
	...
	else if (strcasecmp($display, "Pressure") == 0 OR strcasecmp($display, "Prs") == 0)
		$display = "hourMeasured, pressure, dateMeasured";
	...
	if (($wpdb->num_rows) == 0) //nothing in table 
		...
		if (strcasecmp($display, "*") == 0) {
			if (strcmp($displayMeasurement, "C") == 0)
				$content = "['Time','Temperature [C]','Humidity [%]','Pressure [hPa]'],";
			else
				$content = "['Time','Temperature [F]','Humidity [%]','Pressure [hPa]'],";
	...
	foreach ($resultSet as $row) {
		...
		elseif (strpos($display, "pressure") != 0) //for displaying pressure only
			$content .= "['" . $AxisX . "'," . $row['pressure'] . "],";
		else
			$content .= "['" . $AxisX . "'," . $temperature . "," . $row['humidity'] . $row['pressure'] . "],";
	}
